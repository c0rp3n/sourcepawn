 ...
 */
int
SC3ExpressionParser::skim(int* opstr, void (*testfunc)(int), int dropval, int endval, HierFn hier,
                          value* lval)
{
    ...
    for (;;) {
        ...
        if ((foundop || hits) && (lval->ident == iARRAY || lval->ident == iREFARRAY))
            error(33, lval->sym ? lval->sym->name()
                                : "-unknown-"); /* array was not indexed in an expression */
        if (foundop) {
            if (!hits) {
 ...
 */
int
SC3ExpressionParser::plnge(int* opstr, int opoff, HierFn hier, value* lval, const char* forcetag,
                           int chkbitwise)
{
    int lvalue, opidx;
    int count;
 ...
 */
void
SC3ExpressionParser::plnge2(void (*oper)(void), HierFn hier, value* lval1, value* lval2)
{
    int index;
    cell cidx;
...
SC3ExpressionParser::hier14(value* lval1)
{
    ...
    if (lval1->ident == iARRAYCELL || lval1->ident == iARRAYCHAR || lval1->ident == iARRAY ||
    ...
    if (leftarray) {
        ...
        if ((lval3.ident == iARRAYCELL || lval3.ident == iARRAYCHAR) && lval3.constval > 0 &&
            lval3.sym->dim.array.level == 0)
        {
        ...
        if (lval3.sym->dim.array.level != level)
            ...
            return error(47); /* array sizes must match */
                 ...
                 !matchtag(lval3.sym->x.tags.index, idxtag, MATCHTAG_COERCE | MATCHTAG_SILENT))
            error(229, (lval2.sym != NULL) ? lval2.sym->name()
                                           : lval3.sym->name()); /* index tag mismatch */
        if (level > 0) {
            ...
            for (i = 0; i < level; i++) {
                ...
                if (sym1->dim.array.length != sym2->dim.array.length)
                    error(47); /* array sizes must match */
                                   ...
                                   MATCHTAG_COERCE | MATCHTAG_SILENT))
...
int SC3ExpressionParser::hier10(value* lval)
{ /* ==, != */
...
enum FieldExprResult { FER_Fail, FER_Accessor, FER_CallFunction, FER_CallMethod };
        ...
        if (tok == '[') { /* subscript */
            ...
            if (lval2.ident == iCONSTEXPR) { /* constant expression */
                ...
                if (!(sym->tag == pc_tag_string && sym->dim.array.level == 0)) {
                    /* normal array index */
                    if (lval2.constval < 0 ||
                        (sym->dim.array.length != 0 && sym->dim.array.length <= lval2.constval))
                    ...
                    if (lval2.constval < 0 ||
                        (sym->dim.array.length != 0 &&
                         sym->dim.array.length * ((8 * sizeof(cell)) / sCHARBITS) <=
 ...
 */
void
SC3ExpressionParser::callfunction(symbol* sym, const svalue* aImplicitThis, value* lval_result,
                                  int matchparanthesis)
{
    ...
    if ((sym->usage & uSTOCK) && !(sym->usage & uREAD) && sc_status == statWRITE) {
        error(195, sym->name());
    }
                            ...
                            lval.ident == iARRAYCHAR)
                        {
                            ...
                            while (sym->dim.array.level > 0) {
                                ...
                                if (arg[argidx].dim[level] != 0 &&
                                    sym->dim.array.length != arg[argidx].dim[level])
                                    error(47); /* array sizes must match */
